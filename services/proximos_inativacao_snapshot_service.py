...
def _snapshot_path() -> Path:
    return Path(__file__).resolve().parents[1] / "data" / "proximos_inativacao_snapshot.json"
...
def salvar_snapshot_proximos_inativacao(rows: list[dict[str, Any]], data_ref: date | None = None) -> dict[str, Any]:
...
def carregar_snapshot_proximos_inativacao(data_ref: date | None = None) -> dict[str, Any] | None:
...
def carregar_snapshot_proximos_inativacao_na_data_ou_anterior(data_ref: date | None = None) -> dict[str, Any] | None:
...
def carregar_primeiro_snapshot_proximos_inativacao_mes(ano: int, mes: int) -> dict[str, Any] | None:
...
def listar_datas_snapshot_proximos_inativacao() -> list[str]:
    return listar_datas_snapshot(_snapshot_path())


def rows_snapshot_proximos_inativacao(payload: dict[str, Any] | None) -> list[dict[str, Any]]:
    rows = []
