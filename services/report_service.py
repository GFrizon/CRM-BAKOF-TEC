...
def _build_assunto_relatorio():
    ...
    inativos_181_1095 = (
        db.session.query(func.count(Cliente.id))
        .filter(*filtro_oracle_base, Cliente.ultimo_pedido_oracle.between(limite_1095, limite_181))
        .scalar()
    ) or 0
    ...
    return (
        ...
        f"Inativos: {int(inativos_181_1095)} | "
...
def _montar_insights_basicos(
    ...
    if total_inativos_181_1095 > total_sem_pedidos_90_150 * 3:
...
def build_relatorio_html():
    ...
    insights_html = _montar_insights_basicos(
        ...
        total_inativos_181_1095=total_inativos_181_1095,
    ...
    <div style="font-family:Arial,Helvetica,sans-serif; font-size:16px; line-height:1.45; color:#222; background:#f7fafc; padding:16px;">
      ...
      <table cellpadding="10" cellspacing="0" border="0" style="width:100%; border:1px solid #dbe3ee; margin-bottom:18px; font-size:16px; background:#ffffff; border-radius:8px;">
        ...
        <tr><td style='padding:10px; border-top:1px solid #edf2f7;'>Clientes inativos (181-1095 dias)</td><td style='padding:10px; border-top:1px solid #edf2f7;' align="right"><b>{_kfmt(total_inativos_181_1095)}</b></td></tr>
