...
def _ensure_usuarios_tipo_enum():
    required_values = ["consultor", "supervisor", "televendas", "supervisor_repr", "representante"]
    try:
...
def _ensure_clientes_origem_enum():
    required_values = ["importado_csv", "manual", "oracle_sync"]
    try:
        column_type = _get_column_type("clientes", "origem")
        current_values = _parse_mysql_enum_values(column_type)
        merged_values = list(current_values)
        ...
        if merged_values == current_values:
            logger.info("Campo clientes.origem ja contem os valores necessarios no ENUM")
            return

        enum_sql = ",".join(f"'{value}'" for value in merged_values)
        db.session.execute(
            text(
                "ALTER TABLE clientes MODIFY COLUMN origem "
                f"ENUM({enum_sql}) "
                "NOT NULL DEFAULT 'manual'"
            )
        )
        db.session.commit()
        logger.info("Campo clientes.origem atualizado com ENUM compativel")
    except Exception as e:
        db.session.rollback()
        logger.warning("Erro ao atualizar enum clientes.origem (pode ja estar atualizado): %s", e)
...
def bootstrap_app_database():
    ...
    if not _column_exists("usuarios", "codigo_representante"):
        _run_ddl("ALTER TABLE usuarios ADD COLUMN codigo_representante VARCHAR(20) NULL")
