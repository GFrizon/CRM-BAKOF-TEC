...
def register_oracle_routes(app):
    ...
    def _iniciar_sync_oracle_background():
        ...
        def sincronizar_background():
            try:
                ok = sincronizacao_automatica_diaria(app)
    ...
    def _montar_mapas_consultor_oracle():
        ...
        return mapa_codigo_para_id, mapa_nome_para_id

    def _consultor_pode_ver_cliente_por_categoria(categoria_consultor: str) -> bool:
        mapa_codigo_para_id, mapa_nome_para_id = _montar_mapas_consultor_oracle()
        return consultor_categoria_permitido_para_usuario(
            tipo_usuario=current_user.tipo,
            consultor_cliente=str(categoria_consultor or "").strip(),
    ...
    def _montar_resposta_detalhes_oracle(
        ...
        modo_detalhe_amplo = eh_especial or int(janela_dias or 0) > 730
        ...
        if usar_periodo_fechado:
            pedidos_oracle = get_pedidos_cliente_periodo_oracle(
                ...
                data_fim=data_fim,
                modo_especial=modo_detalhe_amplo,
            )
        else:
        ...
        return jsonify({
            ...
            "dados_oracle_consultados_em": datetime.now().strftime('%d/%m/%Y %H:%M'),
            "janela_dias": janela_dias,
    ...
    def clientes_inativos_oracle_route():
        ...
        if current_user.tipo == 'supervisor' and not supervisor_dev_liberado():
            return resposta_supervisor_dev_obrigatorio()
    ...
    def detalhes_cliente_oracle(cliente_id: int):
        ...
        try:
            ...
            if current_user.tipo == 'consultor' and cliente.consultor_id != current_user.id:
                categoria_consultor = str(cliente.categoria_consultor or '').strip()
                if not categoria_consultor and cd_cliente_resolvido:
                    try:
                        from oracle_service import get_cliente_oracle_por_codigo
                        detalhes_oracle = get_cliente_oracle_por_codigo(cd_cliente_resolvido) or {}
                        categoria_consultor = str(detalhes_oracle.get('consultor') or '').strip()
                    except Exception:
                        categoria_consultor = ''
                if not _consultor_pode_ver_cliente_por_categoria(categoria_consultor):
                    return jsonify({"success": False, "message": "Sem permisṣo para este cliente"}), 403
    ...
    def detalhes_cliente_oracle_por_codigo(cd_cliente: str):
        ...
        try:
            ...
            if current_user.tipo == 'consultor':
                ...
                vem_de_campanha = '/campanhas/' in referer
                if not vem_de_campanha:
                    if cliente and cliente.consultor_id == current_user.id:
                        pass
                    else:
                        categoria_consultor = str((cliente.categoria_consultor if cliente else '') or '').strip()
                        if not categoria_consultor:
                            try:
                                from oracle_service import get_cliente_oracle_por_codigo
                                detalhes_oracle = get_cliente_oracle_por_codigo(cd_cliente_limpo) or {}
                                categoria_consultor = str(detalhes_oracle.get('consultor') or '').strip()
                            except Exception:
                                categoria_consultor = ''
                        if not _consultor_pode_ver_cliente_por_categoria(categoria_consultor):
    ...
    def garantir_cliente_local_oracle():
        ...
        try:
            ...
            cliente_inativo = (
                Cliente.query
                .filter_by(cd_cliente_oracle=cd_cliente, ativo=False)
                .order_by(Cliente.id.desc())
                .first()
            )
            if cliente_inativo:
                cliente_inativo.ativo = True
                cliente_inativo.consultor_id = current_user.id
                cliente_inativo.nome = nome = str(payload.get('nome') or cliente_inativo.nome or f"Cliente Oracle {cd_cliente}").strip()[:200]
                cliente_inativo.telefone = _limpar_texto(payload.get('telefone')) or cliente_inativo.telefone
                cliente_inativo.telefone2 = _limpar_texto(payload.get('telefone2')) or cliente_inativo.telefone2
                cliente_inativo.cnpj = _limpar_texto(payload.get('cnpj')) or cliente_inativo.cnpj
                cliente_inativo.representante_nome = _limpar_texto(payload.get('representante_nome')) or cliente_inativo.representante_nome
                cliente_inativo.categoria_consultor = _limpar_texto(payload.get('categoria_consultor')) or cliente_inativo.categoria_consultor
                cliente_inativo.conceito = _limpar_texto(payload.get('conceito')) or cliente_inativo.conceito
                cliente_inativo.municipio = _limpar_texto(payload.get('municipio')) or cliente_inativo.municipio
                cliente_inativo.uf = _limpar_texto(payload.get('uf')) or cliente_inativo.uf
                cliente_inativo.contato = _limpar_texto(payload.get('contato')) or cliente_inativo.contato
                cliente_inativo.representante_oracle = _limpar_texto(payload.get('representante_oracle')) or cliente_inativo.representante_oracle
                db.session.commit()
                invalidar_caches_listagens_clientes("reativacao de cliente local oracle")
                return jsonify({
                    "success": True,
                    "cliente": {
                        "id": cliente_inativo.id,
                        "nome": cliente_inativo.nome,
                        "telefone": cliente_inativo.telefone or cliente_inativo.telefone2
                    }
                })
