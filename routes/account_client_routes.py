...
def _montar_status_contato_mensal_por_datas(datas_ligacao):
    agora = datetime.now()
...
def register_account_client_routes(app):
    ...
    def api_busca_clientes():
        ...
        try:
            ...
            if aba in ('oracle', 'inativos'):
                if aba == 'inativos' and current_user.tipo not in ('consultor', 'televendas', 'supervisor', 'representante'):
                ...
                for row in clientes_oracle:
                    ...
                    if codigos_representantes_vinculados:
                        codigo_rep_row = _normalizar_codigo_representante(
                            _codigo_representante_de_texto(str(row.get('representante') or ''))
                        )
                        if codigo_rep_row not in codigos_representantes_vinculados:
                            continue
            ...
            if current_user.tipo == 'consultor':
                q = q.filter(Cliente.origem.in_(("manual", "importado_csv")))
            ...
            for c in clientes_todos:
                if codigos_representantes_vinculados and not _cliente_tem_representante_vinculado(
                    c,
                    codigos_representantes_vinculados,
                ):
                    continue
                ...
                status_contato_mensal = _montar_status_contato_mensal_por_datas(
                    [lig.data_hora for lig in ligs if getattr(lig, "data_hora", None)]
                )
                dados = {
                    "id": c.id,
                    "nome": c.nome,
    ...
    def remover_cliente(cliente_id):
        try:
            if current_user.tipo == 'representante':
                return jsonify({"ok": False, "mensagem": "Perfil possui acesso somente leitura."}), 403
