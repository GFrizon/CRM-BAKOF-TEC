...
from routes.clientes_ligacoes.representante_snapshot_readers import (
    carregar_clientes_construtoras_snapshot_representante,
)
...
def _log_perf(app, label, started_at, **extra):
    log_perf(app, "meus-clientes/construtoras", label, started_at, **extra)


def _carregar_base_construtoras_representante(
    *,
    app,
    current_user,
    ...
    clientes_oracle = carregar_clientes_construtoras_snapshot_representante()
    codigos_oracle = [
    ...
    for cliente_oracle in clientes_oracle:
        ...
        dados_cliente = montar_payload_cliente_oracle(
            ...
            origem_padrao="oracle_construtoras",
        )
    ...
    representantes_ordenados, consultores_construtoras, total_construtoras, stats_construtoras = consolidar_dados_grupos(
    ...
    _log_perf(app, "projecao_representante_build", perf_total, total=total_construtoras)
    return {
        "representantes_ordenados": representantes_ordenados,
        "consultores": consultores_construtoras,
        "total": total_construtoras,
        "stats": stats_construtoras,
    }


def render_aba_construtoras(
    *,
    app,
    aba: str,
    request,
    ...
    # Carteira Construtoras: Oracle/snapshot como fonte de verdade.
    perf_total = time.perf_counter()
    ...
    if current_user.tipo == "representante":
        ...
        projecao = carregar_ou_gerar_projecao_representante(
            ...
            carteira="construtoras",
            agrupar_por=agrupar_por_ativo,
            periodo_recencia=periodo_recencia,
            gerador=lambda: _carregar_base_construtoras_representante(
                app=app,
        ...
        representantes_ordenados, consultores_construtoras, total_construtoras, stats_construtoras = filtrar_projecao_agrupada(
        ...
        response = render_template(
            ...
            is_supervisor=current_user.tipo == "supervisor",
            stats={},
            stats_construtoras=stats_construtoras,
            consultores_construtoras=consultores_construtoras,
        ...
        _log_perf(app, "total", perf_total, total=total_construtoras)
        return response

    perf_step = time.perf_counter()
    clientes_oracle = carregar_clientes_construtoras_deduplicados(app.logger)
    _log_perf(app, "carregar_clientes_construtoras", perf_step, total=len(clientes_oracle or []))
    ...
    perf_step = time.perf_counter()
    pagamento_medio_por_cd = carregar_pagamento_medio_representante(codigos_oracle) if codigos_oracle else {}
    ...
    perf_step = time.perf_counter()
    mapa_meses_compra = (
        carregar_meses_compra_representante(codigos_oracle, periodo=periodo_recencia)
        if codigos_oracle else {}
    )
    _log_perf(app, "meses_compra", perf_step, codigos=len(codigos_oracle))
    meses_total_periodo = obter_total_meses_periodo(periodo_recencia)

    agrupar_por_ativo_geral = agrupar_por if agrupar_por in ("representante", "uf", "consultor", "recencia") else "representante"
    representantes_data = {}
    perf_step = time.perf_counter()
    for cliente_oracle in clientes_oracle:
        ...
        if apenas_meus and current_user.tipo not in ("supervisor_repr", "representante", "televendas"):
        ...
        if agrupar_por_ativo_geral == "uf":
            nome_grupo = str(cliente_oracle.get("uf") or "").strip().upper() or "SEM UF"
        elif agrupar_por_ativo_geral == "consultor":
            nome_grupo = (
                str(cliente_oracle.get("consultor") or "").strip()
                or "SEM CONSULTOR"
            )
        elif agrupar_por_ativo_geral == "recencia":
            nome_grupo = NOME_GRUPO_RECENCIA_LIVRE
        else:
            nome_grupo = (
    ...
    representantes_ordenados, consultores_construtoras, total_construtoras, stats_construtoras = consolidar_dados_grupos(
        ...
        chave_sem_grupo=(
            "SEM UF"
            if agrupar_por_ativo_geral == "uf"
            else ("SEM CONSULTOR" if agrupar_por_ativo_geral == "consultor" else "")
        ),
        conceitos_sem_conceito=("SEM CONCEITO", None),
    )
    _log_perf(app, "consolidar_grupos", perf_step, total=total_construtoras)
    perf_step = time.perf_counter()
    periodo_continuidade = periodo_recencia if agrupar_por_ativo_geral == "recencia" else "ano_atual"
    mapa_meses_continuidade = (
        mapa_meses_compra
        if periodo_continuidade == periodo_recencia
        else None
    ...
    _log_perf(app, "continuidade_compra", perf_step, total=total_construtoras)
    if agrupar_por_ativo_geral == "recencia":
        representantes_ordenados = [
            (
                nome_grupo,
                {
    ...
    if lazy_grupo_nome:
        ...
        for nome_grupo, dados_grupo in representantes_ordenados:
            ...
            return render_template(
                ...
                dashboard_tipo=dashboard_tipo,
                visao=visao,
                agrupar_por=agrupar_por_ativo_geral,
                ano_recencia=datetime.now().year,
                periodo_recencia=periodo_recencia,
                lazy_next_offset=next_offset,
                lazy_has_more=next_offset < len(clientes_grupo),
    ...
    _log_perf(app, "template", perf_step, total=total_construtoras)
    _log_perf(app, "total", perf_total, total=total_construtoras)
    return response
